<aside id="left-sidebar-nav">
    <ul id="slide-out" class="side-nav leftside-navigation ps-container ps-active-y {{defaultClass}}">
        <!-- 用户信息 -->
        <li class="user-details pink accent-1">
            <div class="row">
        ...
        <!-- 菜单信息 -->
        <li v-if="menu.hasChild == 'false'" class="bold {{menu.isAct}}" v-for="menu in nconfig.menus">
            <a href="{{menu.url}}" class="waves-effect waves-cyan">
                <i class="{{menu.icon}}"></i>{{menu.name}}
            </a>
        </li>

        <li v-if="menu.hasChild == 'true'" class="no-padding" v-for="menu in nconfig.menus">
